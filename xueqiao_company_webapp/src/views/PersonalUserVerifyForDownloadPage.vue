...
export default {
  ...
  mounted() {
    this.$storeUtil.setClientAppDownloadTabIndex(1);
    ...
    if (this.isLogin) {
      ...
      if (userId && userId > 0) {
        this.gotoClientAppDownloadPage(userId);
      }
    }
  },
  methods: {
    ...
    doLogin() {
      ...
      this.$http.postByPersonalUser("VerifyPersonalUser", params, function(response) {
    ...
    gotoClientAppDownloadPage(companyId) {
      ...
      this.$router.push({
        ...
        params: {
          // companyId: this.verifyResult.companyId,
