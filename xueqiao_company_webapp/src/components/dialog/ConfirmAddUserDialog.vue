      ...
      <div class="dialog-content custom-dialog-content">
        <div>
          ...
          <div class="dialog-item g-form-item dialog-item-hosting">
            ...
            <div class="hosting-item" v-if="userInfo.groupUserList.length < 1">
              <span>-</span>
            </div>
            <div class="dialog-item-value dialog-item-value-hosting" v-else>
              <div class="hosting-item" v-for="(groupUser,index) in userInfo.groupUserList" v-bind:key="index">
                ...
                <div class="hosting-item-key g-form-label">(<span></span>{{getCnRole(groupUser.role)}})</div>
              </div>
            </div>
          </div>
          <div class="g-form-item">
            <span class="local-attention-text attention-text">确认并添加成功后，雪橇会以邮件的形式发送登录信息和客户端下载地址到该用户的邮箱。</span>
          </div>
        </div>
      </div>
