...
public class UpdateCompanyAddress extends WebUserMethod {
	...
	protected BaseResult doUserMethod(WebRequest request, WebUser user) throws Exception {
		...
		if (StringUtils.isBlank(address)) {
			throw new ErrorInfo(ErrorCodeOuter.PARAM_ERROR.getErrorCode(), "地址不能为空");
		}
		int companyId = user.getUserId();

		try {
			MXueqiaoCompany.getInstance().updateAddress(companyId, address);
		} catch (Throwable t) {
