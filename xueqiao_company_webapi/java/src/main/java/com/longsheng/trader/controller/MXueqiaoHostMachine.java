...
public class MXueqiaoHostMachine {
	...
	public void updateRelatedInfoExpiredTime(int companyId, int groupId, int activedEndTimestamp) throws TException {
		HostingRelatedInfo hostingRelatedInfo = queryRelatedInfoById(companyId, groupId);
		if (hostingRelatedInfo == null) {
			throw new ErrorInfo(9011, "fail to find related info(companyId:" + companyId + ", groupId:" + groupId + ")");
		}
		HostingRelatedInfo updateRelatedInfo = new HostingRelatedInfo();
		updateRelatedInfo.setRelatedId(hostingRelatedInfo.getRelatedId());
		updateRelatedInfo.setActivedEndTimestamp(activedEndTimestamp);
		mHostingMachineDaoStub.updateRelatedInfo(updateRelatedInfo);
	}
}
