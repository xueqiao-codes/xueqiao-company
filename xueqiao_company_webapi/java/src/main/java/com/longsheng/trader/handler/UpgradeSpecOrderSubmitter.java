...
public class UpgradeSpecOrderSubmitter implements OrderSubmitter{
    ...
    public Order submit(OrderConfirmInfo orderConfirmInfo) throws ErrorInfo {
        ...
        try {
            ...
            AppLog.e("UpgradeSpecOrderHandler ---- query product fail ---- errorInfo: " + errorInfo.toString());
        } catch (Throwable throwable) {
            AppLog.e("UpgradeSpecOrderHandler ---- query product fail ---- throwable: " + throwable.toString());
        }
        if (product == null) {
        ...
        order.setProductQuantity(orderConfirmInfo.getQuantity());  // 升级商品的价格为每天的价格，所以，数量为升级的天数
        order.setPrice(product.getPrice());
//        order.setCurrencyType(product.getCurrencyType());
        order.setTotalAmount(BigDecimalUtil.multiply(product.getPrice(), orderConfirmInfo.getQuantity())); // 总价 = 价格 * 天数
...
//        }
        ...
        extraInfoMap.put("ProductPurpose", ProductPurpose.UPGRADE.name());
        ...
        try {
            ...
            AppLog.e("UpgradeSpecOrderHandler ---- placeorder fail  errorInfo: " + errorInfo.toString());
        } catch (Throwable throwable) {
            AppLog.e("UpgradeSpecOrderHandler ---- placeorder fail  throwable: " + throwable.toString());
        }
